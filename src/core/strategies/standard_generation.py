...
class StandardGenerationStrategy:
    ...
    def generate(self, model: Any, prompt: str, config: ModelConfig, context_key: str = "none") -> GenerationResult:
        ...
        try:
            ...
            response_cost = self.token_manager.estimate_cost(api_output_tokens, config.model_name, is_output=True)

            self.user_interaction.display_info("Response Analysis:")
            self.user_interaction.display_info(f"- Output tokens: {api_output_tokens}")
            self.user_interaction.display_info(f"- Estimated output cost: ${response_cost:.6f}")

            # Log usage
            self.token_manager.log_usage(config.model_name, api_input_tokens, "standard_prompt", is_output=False)
            self.token_manager.log_usage(config.model_name, api_output_tokens, "standard_response", is_output=True)
            ...
            return GenerationResult(
                ...
                metadata={
                    ...
                    "response_tokens": api_output_tokens,
                    "model": config.model_name,
