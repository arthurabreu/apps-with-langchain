"""
OpenAI model implementation following SOLID principles.
...
class OpenAIModel(ILanguageModel):
    """OpenAI model implementation with dependency injection."""
    ...
    def __init__(
        ...
        """
        Initialize OpenAI model with dependencies.
    ...
    def _validate_config(self) -> None:
        ...
        if not (0.0 <= self.config.temperature <= 2.0):
            raise ModelConfigurationError("Temperature must be between 0.0 and 2.0")
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            self.logger.info(f"Initializing OpenAI model: {self.config.model_name}")
            ...
            self.logger.info("OpenAI model initialized successfully")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize OpenAI model: {e}")
            raise ModelConfigurationError(f"Failed to initialize OpenAI model: {e}")
    ...
    def generate(self, prompt: str, **kwargs) -> GenerationResult:
        ...
        try:
            ...
            self.user_interaction.display_info("Generating using OpenAI...")
