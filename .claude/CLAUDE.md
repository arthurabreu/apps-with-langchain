...
Requires `.env` with `ANTHROPIC_API_KEY` and `HUGGINGFACE_API_KEY` set. Copy `.env.example` if present, or create `.env` manually.
...
| Folder | Purpose |
|--------|---------|
| `src/docs/` | Project documentation and guides |
| `src/data/` | Persistent data, token usage logs, cost tracking |
| `src/logs/` | Application logs |
| `src/samples/` | Sample task files |
| `src/scripts/` | Utility scripts (e.g., cost hook) |
| `src/responses/` | Generated responses and exports |
